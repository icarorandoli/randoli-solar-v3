...
export default function NovoProjetoPage() {
  ...
  const onSubmit = async (data: NovoProjetoForm) => {
    ...
    try {
      ...
      // Salvar o ID do projeto e mostrar modal de aceite
  ...
  const handleTermAccepted = () => {
    ...
    toast({
      ...
      description: "Termo aceito e projeto registrado. Nossa equipe entrará em contato em breve.",
    });
    setLocation(`/portal/projetos/${createdProjectId}`);
  };

  const handleTermClose = () => {
    // Se fechar sem aceitar, avisa mas mantém na página
    setShowTermModal(false);
    toast({
      title: "Aceite pendente",
      description: "Seu projeto foi criado mas o termo ainda não foi aceito. Você pode aceitar acessando o projeto.",
      variant: "destructive",
    });
    setLocation(`/portal/projetos/${createdProjectId}`);
  };
