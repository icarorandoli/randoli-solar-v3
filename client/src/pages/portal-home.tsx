...
import {
  ...
  CreditCard, Sparkles, FileDown} from "lucide-react";
...
export default function PortalHomePage() {
  ...
  const procuracaoUrl = settings?.procuracao_url || "";
  ...
  // Projetos com QR Code gerado mas não pago
  const paymentPendingProjects = projects.filter(p =>
    p.paymentStatus === "pending" && !!(p as any).pixQrCode
  );

  // Ordenar projetos ativos: com atualização recente (últimas 48h) primeiro
  const RECENT_HOURS = 48;
  const isRecent = (p: any) => {
    if (!p.updatedAt) return false;
    const diff = (Date.now() - new Date(p.updatedAt).getTime()) / (1000 * 60 * 60);
    return diff < RECENT_HOURS;
  };
  const activeProjects = projects
    .filter(p => p.status !== "cancelado" && p.status !== "finalizado" && p.status !== "homologado")
    .sort((a, b) => {
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8 space-y-6 max-w-6xl mx-auto">
      ...
      {paymentPendingProjects.length > 0 && (
        <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/5 p-4 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-yellow-800 dark:text-yellow-300 mb-1">
              {paymentPendingProjects.length === 1
                ? "Pagamento aguardando confirmação"
                : `${paymentPendingProjects.length} pagamentos aguardando confirmação`}
            </p>
            <p className="text-xs text-yellow-700/70 dark:text-yellow-400/70 mb-3">
              {paymentPendingProjects.length === 1
                ? "O QR Code PIX foi gerado. Realize o pagamento para liberar seu projeto."
                : "Os QR Codes PIX foram gerados. Realize os pagamentos para liberar seus projetos."}
            </p>
            <div className="flex flex-col gap-2">
              {paymentPendingProjects.map(p => (
                <Link key={p.id} href={`/portal/projetos/${p.id}`}>
                  <div className="flex items-center justify-between bg-yellow-500/10 hover:bg-yellow-500/20 transition-colors rounded-xl px-3 py-2 cursor-pointer group">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-[10px] font-mono text-yellow-700 dark:text-yellow-400 shrink-0">{(p as any).ticketNumber}</span>
                      <span className="text-xs font-semibold text-yellow-800 dark:text-yellow-300 truncate">{p.title}</span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      {p.valor && <span className="text-xs font-black text-yellow-700 dark:text-yellow-400">R$ {p.valor}</span>}
                      <ChevronRight className="h-3.5 w-3.5 text-yellow-600 group-hover:translate-x-0.5 transition-transform" />
                    </div>
                  </div>
                </Link>
              ))}
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between gap-3 mb-3">
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              ...
                              <Badge className={`text-[10px] font-semibold border-none ${STATUS_BADGE_STYLES[project.status] || "bg-muted text-muted-foreground"}`}>
            ...
            <CardContent className="p-5">
              ...
              {procuracaoUrl && (
                <div className="mt-4 pt-4 border-t border-border/40">
                  <a href={procuracaoUrl} download="Modelo-Procuracao.docx" target="_blank" rel="noreferrer">
                    <Button variant="outline" size="sm" className="w-full gap-2 h-9 text-xs font-bold border-primary/20 text-primary hover:bg-primary/5">
